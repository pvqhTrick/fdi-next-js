export type TFlatten<T> = {
  [K in TPath<T>]: GetValueByPath<T, K>;
};

export type GetValueByPath<T, P> = P extends `${infer Key}.${infer Rest}`
  ? Key extends keyof T
 ...
 * ```
 */
export type TupleKeys<T extends ReadonlyArray<unknown>> = Exclude<keyof T, keyof never[]>;
...
export type TPath<T> = T extends unknown ? PathInternal<T> : never;
